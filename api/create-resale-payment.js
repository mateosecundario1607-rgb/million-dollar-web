...
export default async function handler(req, res) {
  ...
  // Buscar el cubo y verificar que esté en venta
  ...
  if (!cubes.length) return res.status(404).json({ error: 'Cube not found' });

  const cube = cubes[0];
  if (!cube.for_sale) return res.status(400).json({ error: 'Cube is not for sale' });

  const askPrice   = parseFloat(cube.ask_price);
  const fee        = parseFloat((askPrice * 0.015).toFixed(2));
  const sellerGets = parseFloat((askPrice - fee).toFixed(2));
